...
const I18N = {
  ...
  "panel.rdist":            { en: "Retrieval distribution", ko: "검색 분포" },
  "panel.rdist.hint":       { en: "node type · query type · retrieval method — schema v1.6", ko: "노드 유형 · 쿼리 유형 · 검색 방법 — 스키마 v1.6" },
  "rdist.node_type":        { en: "Node type", ko: "노드 유형" },
  "rdist.query_type":       { en: "Query type", ko: "쿼리 유형" },
  "rdist.method":           { en: "Retrieval method", ko: "검색 방법" },
...
function renderUtility(u) {
  ...
  // Citation-mode breakdown — shows how CTX items were matched (substring /
  // semantic / tool_use). Rendered when hits_by_mode is present in snapshot.
  let hitsBlock = "";
  const hm = u.hits_by_mode;
  if (hm) {
    const total = (hm.substring || 0) + (hm.semantic || 0) + (hm.tool_use || 0);
    if (total > 0) {
      const subPct  = Math.round((hm.substring || 0) / total * 100);
      const semPct  = Math.round((hm.semantic  || 0) / total * 100);
      const toolPct = Math.round((hm.tool_use  || 0) / total * 100);
      hitsBlock = `<div class="util-split">
        <div class="util-split-head">Citation mode <span class="hint">${esc(total)} hits</span></div>
        <div class="util-split-bar">
          <div class="util-seg text"  style="flex:${hm.substring || 0};">${subPct}% substring</div>
          <div class="util-seg both"  style="flex:${hm.semantic  || 0};">${semPct}% semantic</div>
          <div class="util-seg tool"  style="flex:${hm.tool_use  || 0};">${toolPct}% tool_use</div>
        </div>
      </div>`;
    }
  }
...
// Node-type color map — matches existing `.tag` classes used in contributors panel
...
const QUERY_TYPE_COLOR = {
  KEYWORD:            "#059669",
  HEADING_EXACT:      "#0891b2",
  HEADING_PARAPHRASE: "#6366f1",
  SEMANTIC:           "var(--accent)",
};
const METHOD_COLOR = {
  BM25:   "#059669",
...
function _distBars(dist, colorMap, total) {
  if (!dist) return `<span style="color:var(--text-dim);font-size:11px;">— no data —</span>`;
  const tot = total || Object.values(dist).reduce((a, b) => a + b, 0) || 1;
  return Object.entries(dist).map(([key, count]) => {
    const pct = (count / tot * 100).toFixed(1);
    const color = colorMap[key] || "var(--text-dim)";
    return `
      <div class="b" style="margin-bottom:5px">
        <div class="bucket" style="min-width:160px;font-size:12px">${esc(key)}</div>
        <div class="bar"><div class="bar-fill" style="width:${pct}%;background:${color};opacity:0.85"></div></div>
        <div class="count" style="font-size:12px">${esc(count)} <span style="color:var(--text-dim)">(${pct}%)</span></div>
      </div>`;
  }).join("");
}

function renderRetrievalDist(snap) {
  const host = $("retrieval-dist-body");
  if (!host) return;
  const nd = snap.node_type_dist;
  const qd = snap.query_type_dist;
  const md = snap.retrieval_method_dist;
  if (!nd && !qd && !md) {
    host.innerHTML = `<span style="color:var(--text-dim);font-size:11px;">— no distribution data —</span>`;
    return;
  }
  const totalN = nd ? Object.values(nd).reduce((a, b) => a + b, 0) : 0;
  const totalQ = qd ? Object.values(qd).reduce((a, b) => a + b, 0) : 0;
  const totalM = md ? Object.values(md).reduce((a, b) => a + b, 0) : 0;
  host.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px;align-items:start">
      <div>
        <div style="font-size:12px;font-weight:600;color:var(--text);margin-bottom:8px">${t("rdist.node_type")} <span style="font-weight:400;color:var(--text-dim)">(${totalN.toLocaleString()} retrievals)</span></div>
        <div class="bars">${_distBars(nd, NODE_TYPE_COLOR, totalN)}</div>
      </div>
      <div>
        <div style="font-size:12px;font-weight:600;color:var(--text);margin-bottom:8px">${t("rdist.query_type")} <span style="font-weight:400;color:var(--text-dim)">(${totalQ.toLocaleString()} queries)</span></div>
        <div class="bars">${_distBars(qd, QUERY_TYPE_COLOR, totalQ)}</div>
      </div>
      <div>
        <div style="font-size:12px;font-weight:600;color:var(--text);margin-bottom:8px">${t("rdist.method")} <span style="font-weight:400;color:var(--text-dim)">(${totalM.toLocaleString()} total)</span></div>
        <div class="bars">${_distBars(md, METHOD_COLOR, totalM)}</div>
        <div style="font-size:11px;color:var(--text-dim);margin-top:6px">HYBRID = BM25 + vec-daemon semantic rerank (e5-small)</div>
      </div>
    </div>`;
}
...
function buildContributorsHTML(data, highlightNodeId) {
  ...
  const refStatusMap = {
    yes_text: { label: "✓ used", color: "#4caf50" },
    yes_tool: { label: "✓ tool", color: "#4caf50" },
    yes_both: { label: "✓✓ used", color: "#2e7d32" },
    no:       { label: "✗ not used", color: "#d32f2f" },
  ...
  const items = contribs.map(c => {
    ...
    return `
      <li class="contributor-item" data-node-id="${esc(c.node_id)}"
          style="padding:8px 10px; margin-bottom:6px; border-left:3px solid var(--accent, #4a90e2);
        ...
        <div style="display:flex; align-items:center; gap:8px; font-size:0.9em">
          ...
          <span style="color:${rs.color}; font-weight:600; font-size:0.85em; flex-shrink:0">${esc(rs.label)}</span>
        </div>
  ...
  return promptBlock + header + `<ul style="list-style:none; padding:0; margin:0">${items || '<li style="color:var(--text-dim); font-style:italic; padding:8px">No contributors — this prompt had no direct BM25 matches.</li>'}</ul>`;
}
...
function buildExplainHTML(data, clickedNode) {
  ...
  if (data.token_contributions && data.token_contributions.length > 0 && data.depth === 1) {
    const rows = data.token_contributions.slice(0, 8).map(tc => `
      <tr>
        ...
        <td style="text-align:right">${tc.tf}</td>
        <td style="text-align:right">${tc.idf !== null ? tc.idf.toFixed(2) : "—"}</td>
        <td style="text-align:right"><b>${tc.contribution_pct}%</b></td>
      </tr>`).join("");
  ...
  // UX-optimized order: context → answer → proof → artifact → history
  // SIMPLIFIED 3-section layout (2026-04-24 user feedback):
  //   1. FROM (which prompt) → 2. WHY (this node matched) → 3. WHAT USED (memory + reference)
  // Remaining sections (breakdown table, competitors, missed, content, utility) are
  // folded into a collapsed <details> block below the 3 main sections.
  ...
  return `
    <div class="node-info">
      <span class="tag ${esc(data.node_type)}">${esc(data.node_type)}</span>
    ...
    ${promptSection}
    <div class="explain-section-label" style="font-size:0.75em; color:var(--accent-deep, #4a90e2); letter-spacing:0.5px; text-transform:uppercase; margin-top:10px; margin-bottom:2px">② why — retrieval match</div>
    ${summarySection}
    ${whySection}
    <div class="explain-section-label" style="font-size:0.75em; color:var(--accent-deep, #4a90e2); letter-spacing:0.5px; text-transform:uppercase; margin-top:10px; margin-bottom:2px">③ what used — memory injected for the prompt</div>
    ${injectionSection}
    ${detailsBlock}
  `;
}
...
function _buildCascadeEdges(rawEdges, rootId, maxDepth = 3, maxTotal = 40) {
  ...
  rawEdges.forEach((e, i) => {
    ...
    if (e.type === "recall-d" || e.type === "recall-w") {
  ...
  const step = (tNow) => {
    ...
    for (const e of travelEdges) {
      ...
      for (let k = 0; k < 3; k++) {
        ...
        signalCtx.fillStyle = `rgba(204, 120, 92, ${headAlpha * 0.25})`;
        signalCtx.fill();
        signalCtx.beginPath();
        signalCtx.arc(x, y, baseRadius, 0, Math.PI * 2);
        signalCtx.fillStyle = `rgba(244, 156, 120, ${headAlpha})`;
        signalCtx.fill();
      }
    }
...
function apply(snap) {
  ...
  renderRetrievalDist(snap);
  $("events").innerHTML = renderEvents(snap.recent);
  renderSamples(snap.samples);
  $("thresholds").textContent = fmtThresholds(snap.thresholds) + (snap.schema_version ? `  ·  schema ${snap.schema_version}` : "");
