...
We compare CTX against eight baselines spanning the spectrum from no retrieval to production-grade neural RAG, graph-based retrieval, and a hybrid approach:
...
RANGER-approx & AST-based call+import graph BFS \\
...
RANGER-approx & 0.318 & 0.345 & 0.345 & 5.8 & 0.249 \\
...
CTX achieves a TES of 0.776, which is 1.9$\times$ higher than BM25 (0.410), 3.1$\times$ higher than RANGER-approx (0.249), and 2.6$\times$ higher than Hybrid Dense+CTX (0.303). RANGER-approx, despite using AST-based call graph edges, achieves only 0.345 Recall@5 compared to CTX's 0.874, demonstrating that trigger-driven query routing is more effective than uniform graph traversal. While BM25 achieves higher absolute Recall@5 (0.982 vs.\ 0.874), it consumes 3.6$\times$ more tokens.
...
RANGER-approx & 0.559 & 0.124 & 0.032 & 0.099 \\
...
RANGER-approx & 0.66 & 0.05 & 0.00 & 0.33 \\
...
\subsection{RANGER-approx Comparison}
\label{sec:ranger_comparison}

To directly compare against graph-based SOTA, we implement RANGER-approx: an approximation of RANGER (2025) using Python \texttt{ast} for precise symbol extraction and combined import+call graph BFS traversal.

\begin{table}[t]
\centering
\small
\caption{CTX vs RANGER-approx on synthetic data.}
\label{tab:ranger}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Strategy} & \textbf{R@5} & \textbf{Tok\%} & \textbf{TES} & \textbf{IMPL R@5} \\
\midrule
GraphRAG-lite & 0.514 & 22.5 & 0.214 & 0.43 \\
RANGER-approx & 0.345 & 5.8 & 0.249 & 0.33 \\
\textbf{CTX (Ours)} & \textbf{0.874} & \textbf{5.2} & \textbf{0.776} & \textbf{1.00} \\
\bottomrule
\end{tabular}
\end{table}

CTX outperforms RANGER-approx by 153\% in Recall@5 and 212\% in TES. The key difference is query routing: RANGER-approx applies uniform graph traversal regardless of query type, while CTX classifies queries into trigger types and routes each to a specialized pipeline. On \textsc{Implicit\_Context} queries---the target use case for graph-based methods---CTX achieves perfect recall (1.0) versus 0.33 for RANGER-approx. RANGER-approx's AST-based call graph enrichment provides marginal TES improvement over GraphRAG-lite (0.249 vs 0.214) but cannot compensate for the lack of query-aware retrieval strategy selection.

On RepoBench cross-file retrieval, RANGER-approx achieves R@5 of 1.0 (matching BM25), while CTX achieves 0.483. This reversal highlights CTX's limitation on pure cross-file dependency tasks where the import-based trigger classifier underperforms direct graph traversal. RANGER-approx's AST-based symbol resolution provides stronger seed matching for import-heavy queries.
