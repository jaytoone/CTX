...
**Pilot findings (N=50 MAB + N=10 LongMemEval + N=10 PUAC + faithful claude-mem replication)**: on synthetic conflict-resolution (MemoryAgentBench Competency-4), CTX's BM25-only production retriever achieves 0.40 [0.28, 0.54] (Wilson 95% CI); our `ctx_v2` (Porter stemmer + recency) lifts this to 0.58 [0.44, 0.71] (paired McNemar vs ctx: p=0.049, **formally significant**); a hybrid `ctx_v3` (BM25 + Chroma cascade) reaches **0.92 [0.81, 0.97]**, tying the oracle ceiling at 0.92 [0.81, 0.97] — the hybrid retriever is statistically indistinguishable from perfect retrieval at N=50. When claude-mem's actual pipeline is replicated faithfully (LLM-summarize each session → index summaries in Chroma), accuracy is **0.82 [0.69, 0.90]**, slightly higher than the raw-turn Chroma proxy at 0.78 [0.65, 0.87]. The 10-pp mean gap between `ctx_v3` and `claudemem_faithful` does not reach formal significance at N=50 (McNemar p=0.227) — full N=700 will resolve whether this is a true ceiling or under-powered signal. Notably, the N=10 story where faithful (0.80) scored BELOW proxy (1.00) reversed at N=50 once a bug in Chroma collection caching was fixed; this is itself a methodological finding (§8.8). On *real* LongMemEval (ICLR 2025), all non-BM25-only retrievers tie at 0.30 (CTX at 0.10, indistinguishable from no-memory baseline). The gap between synthetic-favorable (0.58 vs 0.84) and real-data-compressed (0.30 tie) is itself a finding: benchmark-optimized rankings do not transfer to practice. On the PUAC attribution-decomposition axis, mean Causal Lift = +0.207, but 30% of prompts exhibit Wallat-style post-rationalization (high attribution, zero causal benefit). CTX's homograph audit shows 85.8% surface-match-only rate on ambiguous-term queries — validating the architectural need for semantic rerank on top of BM25.
...
| `chroma` (raw-turn proxy) | 0.780 | [0.648, 0.872] | ±0.112 |
| `claudemem_faithful` | 0.820 | [0.692, 0.902] | ±0.105 |
| **`ctx_v3` (hybrid)** | **0.920** | **[0.812, 0.968]** | **±0.078** |
| `oracle` | 0.920 | [0.812, 0.968] | ±0.078 |

The ordering is `ctx_v3` = `oracle` > `claudemem_faithful` > `chroma` > `ctx_v2` > `ctx` > `none`. **`ctx_v3` and `oracle` share identical Wilson 95% CIs** — the hybrid retriever achieves accuracy statistically indistinguishable from perfect retrieval at N=50. Notably, `claudemem_faithful` (0.82) is mean-higher than `chroma` (0.78), reversing the N=10 story where the raw-turn proxy over-estimated; at N=50 the faithful LLM-summarization actually OUTPERFORMS raw-turn indexing by 4 pp, suggesting summarization acts as a noise filter on synthetic haystacks at this scale. **Paired McNemar tests on the same 50 prompts**: `ctx_v3` vs `claudemem_faithful` 8-vs-3 discordant favor `ctx_v3` (p=0.227, NOT significant at α=0.05 — 10-pp mean gap requires N≥150 to cross threshold); `ctx_v3` vs `chroma` 9-vs-2 (p=0.065, MARGINAL); **`ctx_v2` vs `ctx` 13-vs-4 (p=0.049, SIGNIFICANT)** — Porter stemmer + recency improvement is formally confirmed. Full N=700 will tighten the remaining comparisons.
