...
| `claudemem_faithful` | 0.800 | [0.670, 0.888] | ±0.109 |
| `chroma` (raw-turn proxy) | 0.840 | [0.715, 0.917] | ±0.101 |
| **`ctx_v3` (hybrid)** | **0.880** | **[0.762, 0.944]** | **±0.091** |
| `oracle` | 0.920 | [0.812, 0.968] | ±0.078 |

The ordering is `ctx_v3` > `chroma` > `claudemem_faithful` > `ctx_v2` > `ctx` > `none`, with oracle as ceiling. Key CI comparisons: `ctx_v3` [0.762, 0.944] vs `oracle` [0.812, 0.968] overlap substantially — the hybrid retriever is statistically indistinguishable from perfect retrieval at N=50. `ctx_v3` [0.762, 0.944] vs `claudemem_faithful` [0.670, 0.888] have overlapping but shifted CIs; the 8-pp mean gap does not reach formal significance without paired McNemar (planned for full N=700). Critically, `chroma` proxy (0.84) falls BETWEEN `claudemem_faithful` (0.80) and `ctx_v3` (0.88), confirming that raw-turn Chroma OVER-estimates claude-mem's real retrieval by 4 pp (mean) while the true production winner on this axis is the hybrid CTX cascade. `ctx_v2` (0.58) alone cannot close the gap to dense — Porter stemming + recency are necessary but not sufficient for conflict-resolution.
