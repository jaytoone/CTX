...
This hybrid approach addresses CTX's remaining gap on text-to-code semantic matching (COIR R@5 = 0.740 standalone) while preserving structural dependency awareness.
...
On EXPLICIT_SYMBOL and TEMPORAL_HISTORY queries, multiple strategies achieve near-perfect recall, including the new LlamaIndex and Chroma Dense baselines. On SEMANTIC_CONCEPT queries, BM25 dominates due to strong vocabulary overlap; Chroma Dense's neural embeddings capture conceptual similarity moderately well (0.81) but trail keyword methods. The critical distinction emerges on IMPLICIT_CONTEXT queries: CTX achieves perfect recall (1.0), while all seven baselines---including LlamaIndex (0.40), Chroma Dense (0.38), GraphRAG-lite (0.43), and even Hybrid Dense+CTX (0.53)---plateau well below. Neither AST-aware chunking (LlamaIndex), neural embeddings (Chroma), nor partial graph expansion (Hybrid) resolve transitive code dependencies; only fully trigger-classified import graph traversal achieves this.
...
On text-to-code retrieval, neural embedding (MiniLM) achieves perfect Recall@5 and the highest MRR (0.978), while BM25 and TF-IDF achieve near-perfect performance via vocabulary overlap between docstrings and code. CTX Adaptive Trigger achieves R@5 of 0.740 (R@1=0.720, MRR=0.728), a significant improvement over the initial 0.380 achieved before two targeted fixes: (1) tightening the CamelCase pattern in the trigger classifier to require internal uppercase (preventing sentence-initial capitals such as "Return" and "Updates" from being misclassified as EXPLICIT_SYMBOL, and routing them correctly to SEMANTIC_CONCEPT+BM25), and (2) a hybrid BM25 blend in \texttt{\_concept\_retrieve} that mixes concept-term BM25 (0.6w) with full-query BM25 (0.4w) to recover recall lost when concept extraction shortens the query. The remaining gap vs. BM25 (0.740 vs. 0.980) reflects CTX's architecture---its SEMANTIC_CONCEPT branch uses keyword-level BM25 without semantic embedding, which lags behind BM25's full-vocabulary matching on text-to-code queries.

The Hybrid Dense+CTX variant achieves R@5 of 0.950, a 1.28× improvement over CTX alone (0.740→0.950) and close to Dense Embedding (1.000). This validates the two-stage pipeline: dense seed selection bridges the remaining semantic gap that CTX alone cannot fully close, while graph expansion adds structural context beyond what embeddings capture. The hybrid approach represents the best of both worlds for workloads that require both semantic and structural retrieval capabilities.
...
*Full CTX (A) uses an independent benchmark sample; minor run-to-run variation accounts for the difference from main results (Section 4.4: Synthetic R@5=0.874, TES=0.776).*
...
**Trigger classifier accuracy**: Internal analysis of the rule-based trigger classifier on the 166-query synthetic benchmark (proxy ground truth: rule-based labeling) reports 100\% overall accuracy after fixing a pattern that missed snake\_case function identifiers in "Find the function \textit{X}"-style queries (previously classified as SEMANTIC due to missing snake\_case symbol matching). All four trigger types achieve F1=1.00 on the proxy benchmark. However, this measurement uses rule-based proxy labels as ground truth, creating a circular evaluation—so true accuracy on diverse real developer queries is likely lower. A human-annotated evaluation is required to characterize generalization. The ablation in Table 5 confirms that the classifier's primary contribution is TES (token efficiency); correct routing also improves in-distribution R@3 from 0.862 to 0.869 (current system) by directing "Find the function X" queries to the precise symbol index path rather than BM25.
