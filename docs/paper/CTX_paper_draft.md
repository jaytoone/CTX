...
3. **The Trade-off Efficiency Score (TES) metric.** We propose TES = Recall@K / ln(1 + |retrieved|) as a unified measure of the accuracy-efficiency trade-off, capturing the diminishing marginal returns of loading additional context. CTX achieves TES of 0.78 on synthetic data, 40.8x higher than full-context loading (0.019).
...
| **Mean (v4 calibrated)** | | 0.000 | 0.688 | **+0.688** |

*Benchmark calibration note*: The initial v3 benchmark contained two scenarios testing general Python knowledge (directory exclusion conventions, regex character-length rules) that strong LLMs could answer without CTX context—inflating the WITHOUT baseline to 0.333 and depressing Nemotron's $\Delta$ to 0.667. In v4, these were replaced with scenarios requiring exact CTX implementation knowledge: (1) the confidence arithmetic formula `min(0.85, 0.70 + N \times 0.03)` yielding 0.79 for $N=3$, and (2) the 6th entry in `_CONCEPT_EXTRACT_PATTERNS` (`deals?\s+with`). Both replacements produced WITHOUT=0.000 on Nemotron, confirming the v3 leakage was a measurement artifact rather than a CTX limitation.
