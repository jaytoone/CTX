...
1. **A four-type trigger taxonomy for code queries.** We define EXPLICIT_SYMBOL, SEMANTIC_CONCEPT, TEMPORAL_HISTORY, and IMPLICIT_CONTEXT as distinct query categories, each mapped to a specialized retrieval strategy. This classification enables adaptive resource allocation: simple symbol lookups require few files, while dependency queries require graph traversal.
...
The empirical evidence for context dilution is substantial. Shi et al. (2024) document the "Lost in the Middle" phenomenon where LLM accuracy follows a U-curve over document position. Kandpal et al. (2023) show that LLMs struggle disproportionately with long-tail knowledge. LongCodeBench (2025) extends these findings to code, demonstrating that code task accuracy degrades severely beyond 32K tokens. The Head-to-Tail study (NAACL 2024) further characterizes frequency-dependent knowledge retrieval failures. These findings motivate CTX's core design: rather than expanding context windows, reduce context to only what is relevant, guided by code structure.
...
On real data, CTX maintains its token efficiency advantage (1.0--2.1% vs. 4.9--6.2% for baselines), achieving the highest average TES (0.195) across all three real codebases despite lower absolute recall. The TES advantage is most pronounced on larger codebases: on AgentNode (596 files), CTX achieves TES of 0.175 vs. 0.125 for Dense TF-IDF (1.4x), while using 0.4% vs. 0.1% of tokens. On OneViral (299 files), CTX achieves TES of 0.232, the highest among all strategies, 2.5x higher than BM25 (0.065). CTX vs BM25 is statistically significant (McNemar p<0.05) on 3 of 4 datasets.
...
**Benchmark versioning note**: MiniMax M2.5 was evaluated on benchmark v2 and Nemotron on benchmark v4 (calibrated). The mean therefore aggregates results across different benchmark versions. The raw per-LLM deltas ($+0.375$ for MiniMax, $+1.000$ for Nemotron) should be interpreted separately. The mean $\Delta = +0.688$ reflects the arithmetic mean of the two raw deltas (since WITHOUT = 0.000 for both LLMs, $\Delta$ = WITH CTX accuracy in all cases). The v3→v4 calibration corrected two scenarios that tested general Python knowledge rather than CTX-specific implementation details (see Limitations, Section 6).
...
The Hybrid Dense+CTX variant achieves R@5 of 0.950, a 150% improvement over CTX alone (0.380) and close to Dense Embedding (1.000). This validates the two-stage pipeline: dense seed selection bridges the semantic gap that CTX alone cannot cross, while graph expansion adds structural context beyond what embeddings capture. The hybrid approach represents the best of both worlds for workloads that require both semantic and structural retrieval capabilities.
...
2. **Trigger classifier (A vs C):** Without classification, the system uses uniform TF-IDF, achieving higher raw Recall@5 (0.953) but nearly doubling token usage (22.0% vs. 5.2%), which halves TES from 0.780 to 0.406. The classifier is the primary TES driver.
...
CTX and Memori represent fundamentally different approaches to LLM context management. Memori operates on flat embedding spaces, treating all documents as vectors without structural awareness. CTX exploits the import graph---a structure unique to code---to capture dependencies that embeddings cannot represent.
...
**Limitations.** The pass@1 evaluation (Section 4.7.1) uses LLM self-judgment rather than execution-based verification (n=49). The G1/G2 downstream evaluation uses only 2 LLMs and small query sets (n≤10 per scenario); the v4 benchmark recalibration corrects 2 of 6 G2 scenarios that leaked general Python knowledge (WITHOUT inflated to 0.333 on Nemotron), improving G2 Δ from 0.667 to 1.000 on Nemotron and mean Δ from 0.521 to 0.688. One evaluation LLM (Nemotron-Cascade-2) is not publicly available for reproduction.

**Trigger classifier accuracy**: An internal analysis of the rule-based trigger classifier on the 166-query synthetic benchmark (proxy ground truth: rule-based labeling) reports 60.2% overall accuracy. The primary failure mode is SEMANTIC\_CONCEPT recall of 0.083 (66/72 queries misclassified as EXPLICIT\_SYMBOL). However, this measurement uses rule-based proxy labels as ground truth---creating a circular evaluation---and the practical impact may be limited because EXPLICIT\_SYMBOL retrieval on concept-bearing query terms often returns useful results from the symbol index. A human-annotated evaluation of the classifier is a key item for future work. The ablation in Table 5 confirms that the classifier's primary contribution is TES (token efficiency) rather than recall, suggesting that misclassification downgrades efficiency more than accuracy.
